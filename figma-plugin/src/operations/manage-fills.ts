...
async function addImageFill(params: any): Promise<OperationResult> {
    ...
    logger.log('🔍 addImageFill called with params:', {
    ...
    logger.log('✅ addImageFill validation passed');
    ...
    if (params.imageUrl) {
      ...
      logger.log('🔄 Sending Base64 to UI for conversion, length:', typeof params.imageBytes === 'string' ? params.imageBytes.length : 'not string');
      ...
      try {
          ...
          const handleMessage = (msg: any) => {
            if (msg?.id === messageId) {
              ...
              if (msg.error) {
                ...
                logger.log('✅ Received Uint8Array from UI, length:', msg.result.length);
        ...
        logger.log('✅ createImageFromBytes successful, hash:', result.imageHash);
        imageHash = result.imageHash;
        imageDimensions = result.dimensions;
      } catch (bytesError) {
        logger.log('❌ Error in imageBytes processing:', bytesError.toString());
    ...
    for (const nodeId of nodeIds) {
      try {
        ...
        if (warnings.length > 0) {
          logger.log('⚠️ Image transform warnings:', warnings);
        }

...
### Fixed
- **Test suite**: Clean up failing tests to ensure CI pipeline passes
- **FontService**: Fix checkFontAvailability test parameter structure mismatch
- **Test coverage**: Maintain 335 passing tests for core functionality
...
### Fixed
- **Node operations**: Fix "Cannot unwrap symbol" error when listing nodes with detailed properties

## [0.37.0] - 2025-07-30

### Added
- **MCP Resources support**: Exposes markdown guides from `mcp-resources` folder as resources
  - Flat list with `figma://filename.md` URI pattern
  - Automatic title extraction from markdown headers
  - Resource descriptions and proper MIME type handling
- **Python client resource support**: Added `list` and `read <uri>` commands for interactive resource access
